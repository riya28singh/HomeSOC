...
Server: Run python app.py. It will default to http://0.0.0.0:5000.
Finder: Identify your server's local IP (e.g., 192.168.1.10).
Agent: Set SERVER_URL = "http://192.168.1.10:5000/api/logs".
Option B: Public Cloud Deployment (PythonAnywhere)
Best for monitoring hosts across the global internet.

Server: Deploy the server/ folder to PythonAnywhere.
WSGI Setup: Configure your WSGI file to point to server/app.py.
Agent: Set SERVER_URL = "http://YOURUSERNAME.pythonanywhere.com/api/logs".
📦 Installation & Setup
